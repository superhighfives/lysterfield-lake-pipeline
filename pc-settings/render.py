    ...
    while frame_idx < anim_args.max_frames:
        ...
        if prev_sample is not None:
            ...
            if anim_args.color_coherence != 'None':
                ...
                if anim_args.color_coherence == 'Specific':
                    xo_file = args.specific_image
                    print(f'Using color match sample {xo_file}')
                    prev_vid_img = Image.open(os.path.join('/mnt/c/Users/hi/Documents/Dreaming/Colours/', xo_file))
                    prev_vid_img.save(f'{args.outdir}/{xo_file}')
                    with open(f'{args.outdir}/prompt.txt', "w") as text_file:
                        print(f"{cond_prompts}", file=text_file)
                    prev_vid_img = prev_vid_img.resize((args.W, args.H), Image.Resampling.LANCZOS)
                    color_match_sample = np.asarray(prev_vid_img)
                if anim_args.color_coherence == 'Random':
                    xo_files = os.listdir('/mnt/c/Users/hi/Documents/Dreaming/Colours/')
                    xo_file = random.choice(xo_files)
                    print(f'Using color match sample {xo_file}')
                    prev_vid_img = Image.open(os.path.join('/mnt/c/Users/hi/Documents/Dreaming/Colours/', xo_file))
                    prev_vid_img.save(f'{args.outdir}/{xo_file}')
                    with open(f'{args.outdir}/prompt.txt', "w") as text_file:
                    ...
                    prev_vid_img = prev_vid_img.resize((args.W, args.H), Image.Resampling.LANCZOS)
                    color_match_sample = np.asarray(prev_vid_img)
                if anim_args.color_coherence == 'Sticky':
                    prev_vid_img = Image.open(os.path.join(args.outdir, f"{args.timestring}_00000.png"))
                    prev_vid_img = prev_vid_img.resize((args.W, args.H), Image.Resampling.LANCZOS)
                    color_match_sample = np.asarray(prev_vid_img)
